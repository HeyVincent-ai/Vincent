# Vincent - Product Plan

## Overview

Vincent is a secure secret management service designed specifically for AI agents. Instead of giving agents direct access to secrets (like private keys or API credentials), Vincent stores secrets on behalf of users and executes actions using those secrets when requested by authorized agents. The agent never sees the actual secret value.
...
1. **Generated by Vincent** (current focus): For skills like `evm_wallet`, we generate the secret (private key) ourselves. The agent never sees it.
