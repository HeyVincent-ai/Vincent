    ...
    getAdminReferrals()
      ...
      .catch((err) => {
        if (err.response?.status === 403) {
          setError(
            'Admin access required. Your email must be in the ADMIN_EMAILS environment variable.'
          );
        } else {
                ...
                referrals.map((r) => (
                  <tr key={r.id} className="border-b border-border last:border-0 hover:bg-muted/30">
                    <td className="px-4 py-3">
                      <span
                        className={`px-2 py-0.5 rounded text-xs font-medium ${statusBadge(r.status)}`}
                      >
                        {r.status}
                      </span>
                    </td>
