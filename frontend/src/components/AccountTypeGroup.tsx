...
export default function AccountTypeGroup({ label, icon: Icon, accounts }: AccountTypeGroupProps) {
  ...
  return (
    <section className="mb-4">
      ...
      >
        <svg
          className={`w-3 h-3 text-muted-foreground/50 transition-transform ${collapsed ? '-rotate-90' : ''}`}
          fill="none"
          viewBox="0 0 24 24"
          strokeWidth={2.5}
          stroke="currentColor"
        ...
        <span className="text-[10px] text-muted-foreground/40 tabular-nums">{accounts.length}</span>
      </button>
      {!collapsed && (
        <div className="ml-3">
          {accounts.map((a) => (
